...
const Bills = () => {
  ...
  const fetchBills = async () => {
    try {
      const { data } = await axios.get('/financial/bills');
      setBills(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/financial/bills/${editingId}`, form);
        toast.success('Bill updated');
      } else {
        await axios.post('/financial/bills', form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/financial/bills/${id}`);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Link to Purchase Order</label>
                <select
                ...
                >
                  ...
                  {purchaseOrders.map(po => (
                    <option key={po.id} value={po.id}>{po.referenceNo} - {po.supplierName}</option>
                  ))}
                </select>
              </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {bills.map(bill => (
              <tr key={bill.id}>
                <td className="px-6 py-4 whitespace-nowrap font-medium">{bill.referenceNo}</td>
                <td className="px-6 py-4 whitespace-nowrap">{new Date(bill.date).toLocaleDateString()}</td>
                <td className="px-6 py-4">{bill.supplierName}</td>
                <td className="px-6 py-4 whitespace-nowrap">₹{parseFloat(bill.totalAmount).toLocaleString()}</td>
