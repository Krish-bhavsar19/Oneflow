...
const SalesOrders = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/financial/sales-orders/${editingId}`, form);
        toast.success('Sales Order updated');
      } else {
        await axios.post('/financial/sales-orders', form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/financial/sales-orders/${id}`);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold mb-4">
              {editingId ? 'Edit' : 'Create'} Sales Order
            </h3>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {salesOrders.map(so => (
              <tr key={so.id}>
                <td className="px-6 py-4 whitespace-nowrap font-medium">{so.referenceNo}</td>
                <td className="px-6 py-4 whitespace-nowrap">{new Date(so.date).toLocaleDateString()}</td>
                <td className="px-6 py-4">{so.customerName}</td>
                <td className="px-6 py-4 whitespace-nowrap">₹{parseFloat(so.totalAmount).toLocaleString()}</td>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-xs ${statusColors[so.status]}`}>
                    {so.status}
                  </span>
                </td>
